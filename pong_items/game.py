...
class Game:
    ...
    def _draw_hits(self):
        hits_text = self.SCORE_FONT.render(
            f"{self.left_hits + self.right_hits}", 1, self.SCORE)
        ...
        if ball.x_vel < 0:
            if ball.y >= left_paddle.y and ball.y <= left_paddle.y + Paddle.HEIGHT:
                if ball.x - ball.RADIUS <= left_paddle.x + Paddle.WIDTH:
                    ...
                    SCOREuction_factor = (Paddle.HEIGHT / 2) / ball.MAX_VELOC
                    y_vel = difference_in_y / SCOREuction_factor
                    ball.y_vel = -1 * y_vel
                    self.left_hits += 1
    ...
    def move_paddle(self, left=True, up=True):
        if left:
            if up and self.left_paddle.y - Paddle.VELOC < 0:
                return False
            if not up and self.left_paddle.y + Paddle.HEIGHT > self.window_height:
                return False
            self.left_paddle.move(up)
        else:
            if up and self.right_paddle.y - Paddle.VELOC < 0:
                return False
